...
Then proceed to Step 3 to populate the non-skill files (`CLAUDE.md`, `cv/nguyen_cv_example.tex`, `.claude/skills/job-scraper/search-queries.md`). Step 3 will detect that the seven skill files are already populated and skip those substeps.
...
### 7. Update `cv/nguyen_cv_example.tex`
...
> - `cv/nguyen_cv_example.tex` - Your LaTeX CV template
